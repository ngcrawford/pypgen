...
def calculate_raxml_trees(phylip, args, pos):
    ...
    output_file = os.path.join(args.scratch_dir, temp_string)
    ...
    if args.constraint_tree is not None:
        ...
        cli = 'raxmlHPC-PTHREADS-SSE3  \
                ...
                -T {4} \
                -o h665,i02-210 \
                >/dev/null 2>&1'.format(temp_in.name, args.model, constraint_file.name, output_file, args.threads)
        ...
        cli = 'raxmlHPC-PTHREADS-SSE3 \
               -m GTRGAMMA \
                ...
                -T {3} \
                -o h665,i02-210 \
                >/dev/null 2>&1'.format(temp_in.name, args.model, output_file, args.threads)


    print cli
    cli_parts = cli.split()
    ft = Popen(cli_parts, stdin=PIPE, stderr=PIPE, stdout=PIPE).communicate()
    for line in ft:raxml
        print line

    #temp_string = os.path.split(temp_in.name)[1].split('.')[0]

    #treefile = '{}RAxML_result'.format(output_file)
    treefile = "{0}.{1}".format(output_file, "RAxML_result")
    tree = open(treefile, 'r').readlines()[0].strip().strip("\"")

    #statsfile = 'RAxML_info.%s' % (temp_string)
    treefile = "{0}.{1}".format(output_file, "RAxML_info")
    lnL = processStatsFile(open(statsfile, 'r'))

    ## clean up:
    #[os.remove(f) for f in glob.glob("*.%s" % (temp_string))]
    [os.remove(f) for f in glob.glob("%s*" % (output_file))]
...
def calculate_sh_test(phylip, args, pos, constraint_trees, best_tree=None):
    ...
    output_file = os.path.join(args.scratch_dir, run_name)
    ...
    cli = "raxmlHPC-SSE3 \
      ...
      -s {2} \
      -m GTRGAMMA \
      -n {3}".format(best_file.name, const_file.name, temp_in.name, output_file)
    ...
    [os.remove(f) for f in glob.glob("{}*".format(output_file))]
    return (ordered_recalc_const_trees, best_tree_line)



def print_results(lines, best_tree_line, const_trees_dict, chrm, start, stop, args):
   ...
   # chrm, start, stop = re.split(r':|-', args.regions)
    ...
    regions = "{0}:{1}-{2}".format(chrm, start, stop)
    print regions, "best.tre", "GTRGAMMA", best_tree_line[4], chrm, start, stop, best_tree_line[5].strip('\''), 'NA', 'NA'


    args.output.write('\t'.join([str(i) for i in [regions, "best.tre", "GTRGAMMA",
    best_tree_line[4], chrm, start, stop, best_tree_line[5].strip('\''), 'NA', 'NA']]))

    for count, l in enumerate(lines):

        sh_tests = const_trees_dict[l[-1]]
        args.output.write('\t'.join([str(i) for i in [regions, "constraint{}".format(count + 1), "GTRGAMMA", \
...
def do_work(args):
    ...
    for f in glob.glob(args.input):
        ...
        phylip = open(f,'rU').readlines()

        basename = os.path.split(f)[-1]
        chrm, start, stop = basename.split(".")[:3]
        name = "test.run"
        ...
        if args.raxml is True:
            ...
            print "POST CALC TOPOLOGY W/O CONTRAINT"
            print os.listdir(os.curdir)
            ...
            print "RECALCULATE CONSTRAINT WITH BEST CONSTRAINT TREE REMOVED"
            print os.listdir(os.curdir)

            [constraint_trees_dict[i[-1]].append(i[-2]) for i in const_best_tree_results]

            #print_results(const_best_tree_results, best_tree_line, constraint_trees_dict, chrm, start, stop, args)
            #[os.remove(f) for f in glob.glob("RAxML_*.")]


            print "POST FINAL CLEANUP"
            print os.listdir(os.curdir)
...
def main():
    ...
    # SETUP ARGS
    #args = get_args()
    args = Bunch(raxml=False,
                 as_nexus=False,
                 sh_test=True,
                 scratch_dir="/Users/ngcrawford/Desktop/Code/pypgen/scripts/tmp",
                 constraint_tree='((((c511,c512,c513,c514,c515,c563,c614,c630,c639,c640),(p516,p517,p518,p519,p520,p591,p596,p690,p694,p696)),(m523,m524,m525,m589,m675,m676,m682,m683,m687,m689)),(h665,i02-210));((((c511,c512,c513,c514,c515,c563,c614,c630,c639,c640),(m523,m524,m525,m589,m675,m676,m682,m683,m687,m689)),(p516,p517,p518,p519,p520,p591,p596,p690,p694,p696)),(h665,i02-210));((((m523,m524,m525,m589,m675,m676,m682,m683,m687,m689),(p516,p517,p518,p519,p520,p591,p596,p690,p694,p696)),(c511,c512,c513,c514,c515,c563,c614,c630,c639,c640)),(h665,i02-210));',
                 model='GTRGAMMA',
                 threads="2",
                 output=open('test.run.bed','w'),
                 print_header=True,
                 input="/Users/ngcrawford/Desktop/Code/pypgen/test_data/*.phylip")
    do_work(args)
    #[os.remove(f) for f in glob.glob("RAxML_*")]
